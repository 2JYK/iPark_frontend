// 공원 상세 페이지 로드시 세션스토리지에 담은 park 값 html에 적용 
$(document).ready(function () {
	var x = JSON.parse(sessionStorage.getItem("park"))
...
function appendParkHtml(
	id, park_name, addr, check_count, image,
...
function editComment(comment_id) {
  ...
  if (editButton.innerHTML == `<i class="fa-solid fa-pencil"></i>`) {
    ...
    // onkeydown="enterPutComment(${id})" 
    comment.innerHTML = `<textarea class="textarea" id="inputContent(${comment_id})" style="width:100%">${comment.innerText}</textarea>`
